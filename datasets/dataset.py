...
# ==========================================
# DATA READER STRATEGIES (ООП Паттерн)
# ==========================================

class DataReader:
    """Базовый интерфейс для чтения данных (Стратегия)"""
    def infolist(self): raise NotImplementedError
    def namelist(self): raise NotImplementedError
    def open(self, filename, mode='r'): raise NotImplementedError
    def close(self): raise NotImplementedError


class ZipDataReader(DataReader):
    """Стратегия чтения из классических ZIP-архивов"""
    def __init__(self, file_or_path):
        self.zip = zipfile.ZipFile(file_or_path)

    def infolist(self): return self.zip.infolist()
    def namelist(self): return self.zip.namelist()
    def open(self, filename, mode='r'): return self.zip.open(filename, mode)
    def close(self): self.zip.close()


class FolderDataReader(DataReader):
    """Стратегия чтения напрямую из папок (Экономит место на диске)"""
    def __init__(self, directory_path):
        self.directory = Path(directory_path)
        self.parent_dir = self.directory.parent
    ...
    def infolist(self):
        ...
        for root, _, files in os.walk(self.directory):
            for f in files:
                ...
                # Формируем относительный путь (напр. "train2017/0000.jpg")
                rel_name = real_path.relative_to(self.parent_dir).as_posix()
                items.append(DummyZipInfo(rel_name))
        return items
    ...
    def open(self, filename, mode='r'):
        real_path = self.parent_dir / filename
        return open(real_path, 'rb') # 'rb' эквивалентно чтению байтов из архива
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for img_info in sorted(img_zip.infolist(), key=self._sort_key):
            ...
            if target_instance_zip is not None:
                ...
                if check_empty_targets:
                    with target_instance_zip.open(target_instance_filename) as target_instance:
                        ...
                        if all(min_val == max_val for min_val, max_val in extrema):
                            _, labels, _ = self.target_parser(
                                target=tv_tensors.Mask(Image.open(target_zip.open(target_filename))),
                                target_instance=tv_tensors.Mask(Image.open(target_instance)),
    ...
    def __getitem__(self, index: int):
        ...
        if self.targets_instance:
            with target_instance_zip.open(self.targets_instance[index]) as target_instance:
                target_instance = tv_tensors.Mask(Image.open(target_instance), dtype=torch.long)
    ...
    def _get_reader(self, path: Path) -> DataReader:
        """Фабричный метод: выбирает стратегию чтения в зависимости от наличия папки"""
        folder_path = path.with_suffix('')
        if folder_path.is_dir():
            return FolderDataReader(folder_path)
        return ZipDataReader(path)

    def _load_zips(self) -> Tuple[DataReader, DataReader, Optional[DataReader]]:
        worker = get_worker_info()
        worker = worker.id if worker else None

        if self.zip is None: self.zip = {}
        if self.target_zip is None: self.target_zip = {}
        if self.target_instance_zip is None and self.target_instance_zip_path:
            self.target_instance_zip = {}

        if worker not in self.zip:
            self.zip[worker] = self._get_reader(self.zip_path)
            
        if worker not in self.target_zip:
            if self.target_zip_path:
                temp_zip = self._get_reader(self.target_zip_path)
                if self.target_zip_path_in_zip:
                    with temp_zip.open(str(self.target_zip_path_in_zip)) as target_zip_stream:
                        nested_zip_data = BytesIO(target_zip_stream.read())
                    temp_zip.close()
                    # Вложенный ZIP всегда читается через ZipDataReader (из памяти BytesIO)
                    self.target_zip[worker] = ZipDataReader(nested_zip_data)
                else:
                    self.target_zip[worker] = temp_zip
            else:
        ...
        if self.target_instance_zip_path is not None and worker not in self.target_instance_zip:
            self.target_instance_zip[worker] = self._get_reader(self.target_instance_zip_path)
    ...
    @staticmethod
    def valid_member(img_info: Any, img_folder_path_in_zip: Path, img_stem_suffix: str, img_suffix: str):
        return (
    ...
    def close(self):
        if self.zip is not None:
            for item in self.zip.values(): item.close()
            self.zip = None
        if self.target_zip is not None:
            for item in self.target_zip.values(): item.close()
            self.target_zip = None
        if self.target_instance_zip is not None:
            for item in self.target_instance_zip.values(): item.close()
            self.target_instance_zip = None
